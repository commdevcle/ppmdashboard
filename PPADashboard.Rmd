...
FRepl<- mypm.completed %>%
  filter(ProgramType =="Furnace replacement") %>%
  select(ProgramType, ProcessDay, WeekNo,WeekCat)
...
Row
------------------------------------------------------------------------------------------------------------
### Payment process by program type
...
Division of Admin Service
=======================================

row
---------------------------------------
###Social Service Payment Process 
...
graph <- ggplot(mydata, aes(x = WeekCat, y = count_pay)) +
  geom_bar(stat = "identity",
           width = 0.5) +
  ggtitle("Citywide Supporting Grant Program - # of Completed Payment as of today") +
  xlab("# of weeks") + ylab("# of Payment")  + 
  coord_flip() +
  geom_text(aes(label = count_pay),
            hjust = 1.5,
